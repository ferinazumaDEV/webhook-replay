...
- `show`, `list --json` and `curl` mask the values of `Authorization`, `Proxy-Authorization`, `Cookie`, `Set-Cookie` and any header whose name contains `secret`, `token`, `signature`, `hmac`, `password`, `credential`, `authorization`, `api-key` / `api_key`, a `sig` or `auth` segment (`Paypal-Transmission-Sig`, `X-Auth-Key`), or a `private-key` / `access-key` / `subscription-key` / `shared-key` / `client-key` / `session-key` segment (`Ocp-Apim-Subscription-Key`). They print as `<redacted>`, keeping a recognisable scheme prefix where there is one (`Bearer <redacted>`).
